...
def dilation_block_v1(filters, kernel_size, dilation):
    ...
    # Ref: https://github.com/kristpapadopoulos/seriesnet
...
class SimpleWaveNet(object):
    ...
    def __init__(self, target_dim, dec_lens, filters, n_layers, save_dir, dropout=.0, wavenet_mode='v2',
                 loss_fn='mse', optimizer='adam', verbose=0, metrics=['mae']):
        ...
        inputs = Input(shape=(None, target_dim))
        x = inputs
        for l in range(n_layers):
            x, skip_i = cnn_block(filters, 2, 2**l)(x)
            skips.append(skip_i)
        ...
        def _slice(x, seq_length):
            return x[:, -seq_length:, :]

        pred_seq_train = Lambda(_slice, arguments={'seq_length': dec_lens})(out)

        model = Model(inputs, pred_seq_train)
    ...
    def predict(self, x, predict_lens):
        ...
        for step in range(predict_lens):
            pred_i = self.model.predict(x)[:, -1, :]
            x = np.concatenate([x[:, 1:, :], np.expand_dims(pred_i, axis=1)], axis=1)
            preds.append(pred_i)
