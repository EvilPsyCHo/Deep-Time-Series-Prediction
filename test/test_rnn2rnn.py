...
series, series_mean, series_std = normalize(series[:, np.newaxis, DROP_BEFORE:], axis=2)
series_lags = np.nan_to_num((series_lags[:, :, DROP_BEFORE:] - series_mean) / series_std)
...
model.cuda()
learner = Learner(model, opt, './m5_rnn', lr_scheduler=lr_scheduler, verbose=100)
learner.fit(500, trn_dl, val_dl, patient=64, start_save=-1, early_stopping=False)
